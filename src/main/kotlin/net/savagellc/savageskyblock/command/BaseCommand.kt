    ...
    override fun onTabComplete(sender: CommandSender, command: Command, alias: String, args: Array<String>): List<String>? {
        ...
        if (args.size >= 2) {
            ...
            while (commandToTab.subCommands.isNotEmpty()) {
                val findCommand = commandToTab.subCommands.find { subCommand -> subCommand.aliases[0].equals(args[subCommandIndex], true) }
                subCommandIndex++
            ...
            if (commandToTab.requiredArgs.size >= argToComplete) {
                ...
                commandToTab.requiredArgs.addAll(commandToTab.optionalArgs)
                val arg = commandToTab.requiredArgs.find { argument -> argument.argumentOrder == argToComplete } ?: return emptyList()
                val possibleValues = arg.argumentType.getPossibleValues(if (sender is Player) getIPlayer(sender) else null).toMutableList()
