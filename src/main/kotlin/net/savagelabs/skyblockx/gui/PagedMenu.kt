...
abstract class PagedMenu(val pagedMenuConfig: PagedMenuConfig, val fillBackgroundItems: Boolean = true) : BaseMenu(
	fillBackgroundItems,
	MenuConfig(
		pagedMenuConfig.title,
		pagedMenuConfig.backgroundItem,
		pagedMenuConfig.rows,
		pagedMenuConfig.guiMenuItems
	)
) {
	...
	override fun fillContents(player: Player, contents: InventoryContents) {
		...
		contents.set(
			previousPageItem.guiCoordinate.row,
			previousPageItem.guiCoordinate.column,
			ClickableItem.of(previousPageItem.item.buildItem()) {
